 ...
 */
abstract class SpiderWorker(val master:ActorPath,val proxyRouter:ActorPath,val targetKeeper:ActorPath,val serializer:ActorPath) extends SpiderWorkerLike with Crawling{
  ...
  override def receive: Receive = {
    ...
    case M_ELEMENT_RETURN(proxy:JProxy)=>{
      ...
      futureTar.map{
        ...
        case M_ELEMENT_RETURN(tar:String)=>
          self ! M_WORKER_RUN(tar,proxy)
    ...
    case M_WORKER_RUN(target:String,proxy:JProxy)=>{
      try{
        ...
        case e:Exception =>
          ...
          context.system.actorSelection(master) ! M_WORKER_FAILED(RetryEntry(target,0),proxy)
      }
